...
const S = {
  ...
  tableHeader: {
    display: 'grid',
    gridTemplateColumns: '1fr 80px 80px 90px 36px',
    gap: 0,
    background: '#1A1A1A',
  ...
  // Two-col layout for form + result
  twoCol: {
    display: 'grid',
...
export default function ElektroRegiezettel() {
  ...
  // ── PDF generation (client-side, after payment) ──
  const handlePDFDownload = async () => {
    ...
    autoTable(doc, {
      ...
      columnStyles: {
        ...
        1: { halign: 'right', fontStyle: 'bold', textColor: [26, 26, 26] },
      },
    })
  ...
  return (
    <div style={S.page}>
      ...
      {showPaywall && (
        <div style={S.overlay} onClick={() => setShowPaywall(false)}>
          <div style={S.modal} onClick={e => e.stopPropagation()}>
            ...
            <button
              style={{ ...S.btnPrimary, fontSize: '1rem' }}
            ...
            <p style={S.priceNote}>
              Sichere Zahlung via Stripe · Visa, Mastercard, PayPal, Klarna
            </p>
          </div>
        </div>
      )}
      ...
      {/* Hero */}
      <div style={S.hero}>
        {/* Decorative voltage symbol */}
        <div style={{
          position: 'absolute', right: '5%', top: '50%', transform: 'translateY(-50%)',
          fontSize: '14rem', fontWeight: 900, color: 'rgba(245,197,24,0.05)',
        ...
        <div style={S.heroInner}>
          ...
          <p style={S.heroSub}>
            Fahrtzeit, Stundenlohn, Material-Aufschlag und MwSt automatisch kalkuliert. 
            Einmal eingeben — druckfertiges PDF in Sekunden.
          </p>
          ...
          <div>
            ...
            <div style={S.section}>
              ...
              <div style={S.tableWrap}>
                ...
                {data.positionen.map(p => (
                  <div key={p.id} style={S.tableRow}>
                    <input
                      ...
                      onChange={e => setPos(p.id, 'beschreibung', e.target.value)}
                      placeholder="Materialbezeichnung..."
                    />
                    <input
                      style={{ ...S.tableNumInput }}
                      type="number"
                      value={p.menge}
                      onChange={e => setPos(p.id, 'menge', parseFloat(e.target.value) || 0)}
                    />
                    <input
                      style={{ ...S.tableInput, fontSize: '0.8rem' }}
                      value={p.einheit}
                      onChange={e => setPos(p.id, 'einheit', e.target.value)}
                    />
                    <input
                      style={{ ...S.tableNumInput }}
                      type="number"
                ...
                <button style={S.addBtn} onClick={addPos}>
                  + Position hinzufügen
                </button>
              </div>
            </div>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.3rem' }}>
      ...
      <input
        style={{
          background: '#FFFFFF',
          border: `1px solid ${focused ? '#F5C518' : '#D8D0C4'}`,
          padding: '0.6rem 0.75rem',
          fontSize: '0.9rem',
          fontFamily: "'Barlow', sans-serif",
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.3rem' }}>
      <label style={{
        ...
        textTransform: 'uppercase', color: '#888', lineHeight: 1.3,
      }}>{label}</label>
      <input
        type="number"
        step={step}
        style={{
          ...
          fontSize: '1rem',
          fontFamily: "'Barlow Condensed', sans-serif",
          fontWeight: 600, color: '#1A1A1A', outline: 'none', width: '100%',
